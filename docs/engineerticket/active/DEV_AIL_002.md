...
智慧整理後端與模型介面。本次授權先完成分支與設計對接，DEV/QA 依賴滿足後由 PM 排程。
...
DSG_AIL_001 完成且 PM scope review。工作位置與跨分支狀態以 [BRANCH-MAP](../BRANCH-MAP.md) 為準。

## In Scope / Allowed Files

新 app/api/layout/**；app/lib/server/layout-*；runtime capability 必要 hunk；package/lock 僅獲准 SDK；對應 DEV tests

## Acceptance Criteria

- [ ] auth/scope 優先；最小外送；受控 provider mock；錯誤/timeout/cancel/token limit；無 secret；local 通道按設計明確處理
