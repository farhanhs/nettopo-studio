...
- [ ] QA fixture 會建立完整 synthetic site/user/customer/topology 前置資料。
- [ ] 空站點清單時新增拓樸可完成，顯示為「未指定站點」。
- [ ] 有站點清單時新增拓樸可選站點並正確保存。
- [ ] Pilot site scope 與 Engineer 權限未退步。
- [ ] Browser 截圖、network/API result、DB snapshot 均有證據。
- [ ] finally cleanup 後 QA fixture 殘留計數為 0。
