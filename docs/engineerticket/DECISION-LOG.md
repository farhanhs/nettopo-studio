...
## D29 — 修正可執行，Commit 前人工批准（2026-09-04）

- ID：`PM_GOV_001-D29`；Status：`APPROVED`；來源：使用者要求各組繼續，但在 commit point 前停下等 approve。
- 授權本輪 migration checksum 退件的設計、最小修正、獨立 pre-commit 測試及文件回報。
- 從本決策起，暫停 D24/D26/D27/D28 中自動建立後續 commit／push 的執行授權；保留既有技術及安全要求。既有 17 commits 不重寫。
- 所有組別禁止新增 staging、commit、amend、cherry-pick、rebase、merge、push、tag 或 deploy；不以 Git 權限批准代替使用者對變更內容的批准。
- 修正保留為 unstaged diff。完成設計／DEV／QA 後，由 PM 提交變更摘要、完整測試結果、diff 與預定 commit 清單，停止等待使用者明確批准。
- 即使 pre-commit QA 通過，也不自動標記已提交 checkpoint release 通過；批准後仍需驗證實際 commit/tree。
...
- 執行結果：三段 commits 已建立並通過 committed-tree regression；2026-09-08 非 force push 至 `origin/codex/dev-rel-001-checkpoint`，HEAD `1b0b084518886cda347f175bfc959401fd1d18ed`；未 merge、未 deploy。

## D31 — 2026-09-08 UI／Canvas Bug 分流與發單

- ID：`PM_GOV_001-D31`；Status：`APPROVED`；來源：使用者要求 PM 先檢查四項 UI／拓樸問題，再建立工單交相應組別，不直接跳過 PM gate 修復。
- Inspector 切換資料殘留、topbar 中等寬度重疊與登入後 canvas 初始尺寸錯位，均屬已核准 UI/controlled-view/響應式語意的 `CORRECT`；分別建立 `DEV_UIX_006/007/008` 及獨立 QA 票。
- 不重用已完成的 `DEV_UIX_004/005`：004 是 session profile 遮擋 Inspector actions；005 是 drag persistence。新票須回歸這兩張已通過能力。
- 線路交叉／重疊最小化超出既有 obstacle-safe contract，屬 `EXTEND`；先執行 `DSG_UIX_003`，不得直接 coding 或承諾全域最佳解。後續為 `DEV_UIX_009`，fallback 文案沿用並收斂既有 `DEV_UIX_003`，共同交 `QA_UIX_005`。
- 本決策只批准建立、排序與派發 Ticket；不授權本輪直接修改產品、commit、push、merge 或 deploy。各票仍依狀態與 dependency gate 推進。
